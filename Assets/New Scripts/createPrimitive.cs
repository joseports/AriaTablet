...
namespace Assets.New_Scripts
{
    ...
    {
        [HideInInspector]
        // private Rigidbody primRB;
        public bool baseProvided;

        public bool isSelected = false;

        [HideInInspector] private Mesh planeMesh;

        [HideInInspector]
        /// private BoxCollider primColl;
        private MeshCollider primColl;

        /// </summary>
        [HideInInspector] private GameObject primGO;

        //public static GameObject primGO;
        [HideInInspector] private MeshFilter primMFilter;

        [HideInInspector] private MeshRenderer primMRend;

        public GameObject BoxPrefab { get { return primGO;} }
        ...
        public GameObject CreateBox(List<Vector3> inPoints, Material mat)
        {
            ...
            //primGO = SpawnFactory.Spawn("Prefabs/ProceduralBox", Vector3.zero, Quaternion.identity);
            primGO = new GameObject("NewProceduralBox");

            primMFilter = primGO.GetComponent<MeshFilter>();
            primMRend = primGO.GetComponent<MeshRenderer>();
            primColl = primGO.GetComponent<MeshCollider>();

            primMFilter = primGO.AddComponent<MeshFilter>();
            primMRend = primGO.AddComponent<MeshRenderer>();
            ...
            //boxMRend.material.color = Color.blue;
            primMRend.material = mat;

            // isGrabbable = true;
            // isUsable = true;
            // highlightOnTouch = true;
            //grabAttachMechanic=tracked
            primColl.isTrigger = false;
            //primColl.center = new Vector3(0.0f, 0.0f, 0.0f);
            //  primColl.size = new Vector3(1.0f, 1.0f, 1.0f);

            /*
        primRB.mass = 1;
        primRB.drag = 0;
        primRB.angularDrag = 0.1f;
        primRB.useGravity = false;
        primRB.isKinematic = true;
        primRB.constraints = RigidbodyConstraints.FreezePositionX | RigidbodyConstraints.FreezePositionZ | RigidbodyConstraints.FreezePositionY;
        primRB.constraints = RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationZ | RigidbodyConstraints.FreezePositionY;
        ...
        */
        ...
        public GameObject CreatePrism(List<Vector3> points, Material mat)
        {
            ...
            if (isSize)
            {
                ...
                //Debug.Log("creating box");
                var p4 = new Vector3(points[0].x, points[0].y, points[0].z);
                var p5 = new Vector3(points[1].x, points[1].y, points[1].z);
                var p6 = new Vector3(points[2].x, points[2].y, points[2].z);
                //Vector3 p7 = new Vector3(points[3].x, points[3].y, points[3].z);

                var p0 = new Vector3(points[0].x, 0, points[0].z);
                var p1 = new Vector3(points[1].x, 0, points[1].z);
                var p2 = new Vector3(points[2].x, 0, points[2].z);
        ...
        public GameObject CreateCone(List<Vector3> inPoints, Material mat)
        {
            ...
            boxMesh.Clear();
            var centre = new Vector3(0.0f, 0.0f, 0.0f);
            ...
            while (vert <= nbSides)
            {
                ...
                vertices[vert] = new Vector3(centre.x + Mathf.Cos(rad)*bottomRadius, 0f,
                    centre.z + Mathf.Sin(rad)*bottomRadius);
            ...
            while (vert <= nbSides*2 + 1)
            {
                ...
                vertices[vert] = new Vector3(centre.x + Mathf.Cos(rad)*topRadius, centre.y,
                    centre.z + Mathf.Sin(rad)*topRadius);
                vert++;
            }
            ...
            while (vert <= vertices.Length - 4)
            {
                ...
                vertices[vert] = new Vector3(centre.x + Mathf.Cos(rad)*topRadius, centre.y,
                    centre.z + Mathf.Sin(rad)*topRadius);
                //vertices[vert] = new Vector3(Mathf.Cos(rad) * topRadius, height,  Mathf.Sin(rad) * topRadius);
                vertices[vert + 1] = new Vector3(centre.x + Mathf.Cos(rad)*bottomRadius, 0,
                    centre.z + Mathf.Sin(rad)*bottomRadius);
        ...
        public GameObject CreateTube(List<Vector3> inPoints, Material mat)
        {
            ...
            var external = new Vector3(points[1].x, points[1].y, points[1].z);
            ...
            var bottomRadius1 = 0f;
            var bottomRadius2 = 0f;
            var topRadius1 = 0f;
            var topRadius2 = 0f;
            ...
            var nbVerticesCap = nbSides*2 + 2;
            var nbVerticesSides = nbSides*2 + 2;
            ...
            while (vert < nbVerticesCap)
            {
                ...
                var r1 = (float) sideCounter++/nbSides*_2pi;
                ...
                vertices[vert] = new Vector3(centre.x + cos*(bottomRadius1 - bottomRadius2*.5f), 0f,
                    centre.z + sin*(bottomRadius1 - bottomRadius2*.5f));
                vertices[vert + 1] = new Vector3(centre.x + cos*(bottomRadius1 + bottomRadius2*.5f), 0f,
                    centre.z + sin*(bottomRadius1 + bottomRadius2*.5f));
                vert += 2;
            }
            ...
            while (vert < nbVerticesCap*2)
            {
                ...
                vertices[vert] = new Vector3(centre.x + cos*(topRadius1 - topRadius2*.5f), height,
                    centre.z + sin*(topRadius1 - topRadius2*.5f));
                vertices[vert + 1] = new Vector3(centre.x + cos*(topRadius1 + topRadius2*.5f), height,
                    centre.z + sin*(topRadius1 + topRadius2*.5f));
            ...
            while (vert < nbVerticesCap*2 + nbVerticesSides)
            {
                ...
                vertices[vert] = new Vector3(centre.x + cos*(topRadius1 + topRadius2*.5f), height,
                    centre.z + sin*(topRadius1 + topRadius2*.5f));
                vertices[vert + 1] = new Vector3(centre.x + cos*(bottomRadius1 + bottomRadius2*.5f), 0,
                    centre.z + sin*(bottomRadius1 + bottomRadius2*.5f));
            ...
            while (vert < nbVerticesCap)
            {
                var t = (float) sideCounter++/nbSides;
